...
public class CultivableField {
    ...
    public void harvestSystem(MainController game) {
        if (!this.isOccupied) {
            ...
            if (selected.equals("wheat") && game.inventory.wheatSeeds > 0) {
                game.inventory.wheatSeeds -= 1;
                plant(new Wheat());
            } else if (selected.equals("watermelon") && game.inventory.waterMelonSeeds > 0) {
                game.inventory.waterMelonSeeds -= 1;
                plant(new WaterMelon());
            }
        } else if (this.isReady) {
            if (currentPlant instanceof Wheat) {
                game.inventory.wheatStock += 2;
                game.inventory.wheatSeeds += 1;
            } else if (currentPlant instanceof WaterMelon) {
                game.inventory.waterMelonStock += 1;
            }
            resetField();
        }
    }
