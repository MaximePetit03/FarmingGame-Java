...
public class MarketController {
    ...
    @FXML private void selectCow() {
        if (!mainGame.cowUnlocked) {
            market.selectProduct("models.Cow", 1000, 0);
            itemNameLabel.setText("Vache (VERROUILLÉE)");
        } else {
            market.selectProduct("models.Cow", 150, 75);
    ...
    private void selectChicken() {
        if (mainGame != null) {
            if (!mainGame.chickenUnlocked) {
                market.selectProduct("models.Chicken", 500, 0);
                itemNameLabel.setText("Poulet (VERROUILLÉ)");
            } else {
                market.selectProduct("models.Chicken", 80, 40);
    ...
    private void selectGoldenChicken() {
        if (mainGame != null) {
            if (!mainGame.goldenChickenUnlocked) {
                market.selectProduct("models.GoldenChicken", 5000, 0);
                itemNameLabel.setText("Poulet Doré (VERROUILLÉ)");
            } else {
                market.selectProduct("models.GoldenChicken", 1000, 500);
    ...
    private void handleUnlock(int price, String type) {
        if (mainGame.inventory.money >= price) {
            mainGame.inventory.money -= price;
    ...
    @FXML private void confirmPurchase() {
        if (mainGame != null) {
            ...
            if ("models.Cow".equals(currentProduct) && !mainGame.cowUnlocked) {
            ...
            } else if ("models.Chicken".equals(currentProduct) && !mainGame.chickenUnlocked) {
                handleUnlock(500, "chickenUnlocked");
            } else if ("models.GoldenChicken".equals(currentProduct) && !mainGame.goldenChickenUnlocked) {
    ...
    private void confirmSale() {
        if (mainGame != null) {
            ...
            if (currentProduct != null && (currentProduct.contains("Cow") || currentProduct.contains("Chicken"))) {
                removeAnimals();
            }

            mainGame.saveGame();
            updateUI();
            mainGame.update(); // Très important pour rafraîchir l'affichage global
        }
    }

    private void removeAnimals() {
        Animal[] animals = mainGame.animals;

        for (int i = 0; i < animals.length; i++) {
            if (animals[i] != null) {
                if (animals[i] instanceof Cow && mainGame.inventory.cowInventory <= 0) {
                    animals[i] = null;
                }
                else if (animals[i] instanceof Chicken && !(animals[i] instanceof GoldenChicken)
                        && mainGame.inventory.chickenInventory <= 0) {
                    animals[i] = null;
                }
                else if (animals[i] instanceof GoldenChicken && mainGame.inventory.goldenChickenInventory <= 0) {
                    animals[i] = null;
                }
            }
        }

        if (mainGame.animalAreaController != null) {
            mainGame.animalAreaController.updateUI();
        }
    }
    ...
    private void updateUI() {
        ...
        if (confirmPurchaseBtn != null && mainGame != null) {
            if (market.getTotalBuyPrice() > mainGame.inventory.money) {
